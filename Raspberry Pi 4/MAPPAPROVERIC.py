...
class Cell:
	...
	def new_cell(self, muoviamoci, posizione):
		#print('new_cell1', muoviamoci, posizione)
		index = grid_cells.index(self)
		#print('new_cell2', index)
		movimento = muoviamoci[0] 
		#print('new_cell3', movimento)
		...
		if movimento == 'avanti':
			...
			while True:
				...
				if muoviamoci[1]!='gira a destra' and mov.laser.read(5)<120 and mov.laser.read(3)>220 and mov.laser.read(4)>220:  # no len perchè dopo destra sempre altro
					...
					mov.dritta()
				print('fine while gira a destra')
				break
	...
	def vittima(self, vittima_s, vittima_d, posizione, before_cell):
		...
		if not self.placca['nero']:	 # no vittime nella placca nera
			if vittima_s >= 7 and vittima_s <= 9 and self.coo_v[0] == 0 and before_cell.ultima_vittima(vittima_s) and mov.laser.read(4)<140 and mov.laser.read(1)<140:
				trovato = True
			...
			elif vittima_d >= 10 and vittima_d <= 12 and self.coo_v[0] == 0 and before_cell.ultima_vittima(vittima_d) and mov.laser.read(3)<140 and mov.laser.read(2)<140:
				trovato = True
				...
				with open('fermo.txt', 'w') as file:
					file.write(str(False))
			elif vittima_s >= 1 and vittima_s <= 3 and self.coo_v[0]== 0 and before_cell.ultima_vittima(vittima_s) and mov.laser.read(4)<140 and mov.laser.read(1)<140:
				trovato = True
				...
				with open('fermo.txt', 'w') as file:
					file.write(str(False))
			elif vittima_d >= 4 and vittima_d <= 6 and self.coo_v[0]== 0 and before_cell.ultima_vittima(vittima_d) and mov.laser.read(3)<140 and mov.laser.read(2)<140:
				trovato = True
...
def camere(queue):
	...
	while True:
		...
		trovato = current_cell.vittima(colore_s, colore_d, posizione, before_cell)
		print(colore_s, colore_d)
		#print('trovato_colore', trovato)
		if not trovato:
			lettera_s, value_s, lettera_d, value_d = exe_letter_detection()  # value non utili
			print('DESTRA', lettera_d, value_d)
			print('SINISTRA', lettera_s, value_s)
			trovato = current_cell.vittima(lettera_s, lettera_d, posizione, before_cell)
		#print('fine check tele')		
		#'''
		'''	
	...
	while True:
		...
		if (GPIO.input(start) == True) and not flag:
			...
			if len(muoviamoci) != 0:
				...
				#'''
				if "destra" in check or "sinistra" in check:
					placche_d, placche_s = exe_placche_detection()
					print('PLACCAAA', placche_d, placche_s)
					if placche_d == 'si':
						if posizione == 0:
							cell_black = current_cell.check_cell(current_cell.x + 1, current_cell.y)
						elif posizione == 180:
							cell_black = current_cell.check_cell(current_cell.x - 1, current_cell.y)
						elif posizione == 90:
							cell_black = current_cell.check_cell(current_cell.x, current_cell.y - 1)
						elif posizione == 270:
							cell_black = current_cell.check_cell(current_cell.x, current_cell.y + 1)
						cell_black.visited = True
						cell_black.placca['nero'] = True
						#print(placche_d, grid_cells.index(cell_black))
					if placche_s == 'si':
						if posizione == 0:
							cell_black = current_cell.check_cell(current_cell.x - 1, current_cell.y)
						elif posizione == 180:
						...
						cell_black.visited = True
						cell_black.placca['nero'] = True
						#print(placche_s, grid_cells.index(cell_black))
				 #'''
